...
Surrogate is a restful resource Ajax "hijacker" that observes Ajax requests matching specific methods (get, put, post, delete) and paths.  It provides the ability to attach specific behaviors to any request that matches the given criteria.  It's designed to work the way Rails works by giving you a simple methodology for observing and handling remote forms and remote links.  It provides hooks for all the default controller actions (index, show, new, create, edit, update, and destroy), as well the ability to add members (get, put, post, delete actions) to a given resource.  It's intended to provide all that, but isn't solely restricted to it.

It's important to note that there's no requirements to get the routes or anything from rails, it expects you to define only what you want, where you want it.

h2. The Story

I absolutely can't stand RJS and refuse to use it.  I prefer to keep all my Javascript in one place -- outside of my application, and structured in a sensible way for _Javascript_.  Javascript doesn't always retain a one to one relationship with server code, and because of that it seems important to provide simple and elegant ways to get them to interact nicely.
...
Simple and concise.. Now whenever a remote link is clicked with the path of /posts/1 and method of GET (PostsController#show) we get to handle it.  We can also put this code where it seems appropriate in the context that we might need it.  The best part is this is just the simplest example.

h2. Installation

It's expected that you have the default Rails setup (eg. prototype.js, and optionally rails.js) for now.. Support for jQuery and others will be added shortly.

* Grab "surrogate.js":http://github.com/jejacks0n/surrogate/raw/master/public/distro/surrogate.js from the @public/distro@ path.
* Optionally grab "prototype.js":http://github.com/jejacks0n/surrogate/raw/master/public/distro/lib/prototype.js from @public/distro/lib@.
* Put them in your project and include them in your layout.
...
Surrogate was written to be pretty flexible in how you implement using it.  It supports chaining, passing options, etc.. The following is a breakdown of how it can be used.
...
Surrogate handles namespaces and nested resources.  It's just part of the resource you pass it, so for example, if I had something like posts/comments, I simply use the path to that resource.. eg. 'posts/:post_id/comments'
...
Singular resources basically work the same way everything else does.  Surrogate doesn't really care if it's a singular resource or not.  What this means is that if you have a singular resource, the "show" actions is really the "index" action.. So instead of using show for singular resources just use index.
