...
Singular resources basically work the same way everything else does.  The second argument of Rails.resource is a boolean for singular.  The only real difference with singular resources is that they have a show action where normally there's an index action, and there is no index action.  To specify something is a singular resource use:

<pre>
  Rails.resource('posts/:post_id/author', true).
    add('show', function() { alert('This will monitor GET to /posts/1/author') });
</pre>
