...
Surrogate is a small (~100 lines) RESTful resource Ajax "hijacker" that observes Ajax requests matching specific methods (get, put, post, delete) and paths.  It's designed to work with Rails by providing hooks for all the default controller actions (index, show, new, create, edit, update, and destroy), as well the ability to add members (get, put, post, delete actions) to a given resource.  It's intended to provide all that, but isn't solely restricted to it.

It's important to note that there's no requirements to get the routes or anything from rails, it expects you to define only what you want, where you want it -- all you have to do is use @link_to :remote => true@ (or form_for).
